...
First, load the project's always-apply **conventions** in full with `{{config.command}} knowledge conventions`. This returns every convention's body, tagged with the scope it came from (e.g. `project`, `team`, `global`). Read all of them — these are the standards every plan must reflect, and the next step selects the relevant subset into the plan.

Then pull in topic-specific knowledge for the **design surfaces** this feature introduces — the concrete areas the work touches (for example `database`, `auth`, `http-middleware`, `cli`). For each surface, run a targeted `{{config.command}} knowledge search <surface>` rather than one broad "anything about this area" sweep; this keeps research focused as the knowledge base grows. Results come back ranked — one per matching document, strongest match first, each tagged with its scope and carrying a title, score, and excerpts; read a promising one in full with `{{config.command}} knowledge read --data '{"scope":"<scope>","path":"<path>"}'`. Conventions are deliberately excluded from search results (you already loaded them in full above), so search surfaces only reference knowledge — architecture notes, gotchas, prior learnings. If something relevant exists, read it before investigating; it may already answer your questions or flag dead ends. Nothing is required to exist — the knowledge sources can be empty.
