...
Before proceeding to the overview step, write the current conversation context to `.spektacular/context.md` if meaningful context exists. The file already opens with a CLI-managed `## Repos` section listing each registered repo and where its code lives; leave that section in place and write your context below it. Capture the full discussion in detail:
...
If no meaningful context exists (e.g., the user simply said "create a spec for X" without elaboration), add nothing below the `## Repos` section.
